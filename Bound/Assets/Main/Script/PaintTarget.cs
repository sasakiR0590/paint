...
{
    [NonSerialized] public Texture NormalmapTexture;
    ...
    [NonSerialized] public bool SetupOnStart = false;
    [NonSerialized] public bool PaintAllSplats = false;

    [NonSerialized] public PaintDebug debugTexture = PaintDebug.none;

    private Camera renderCamera = null;

    private RenderTexture splatTex;
    private RenderTexture splatTexAlt;
    [NonSerialized] public Texture2D splatTexPick;
    ...
    static RenderTexture rt_2;
    [NonSerialized] public Renderer r;

    public MaScTes testScript = null;
    ...
    public static void PaintObject(PaintTarget target, Vector3 point, Vector3 normal, Brush brush)
    {
        ...
        float randScale = UnityEngine.Random.Range(brush.splatRandomScaleMin, brush.splatRandomScaleMax);
        splatObject.transform.RotateAround(point, normal, brush.splatRotation);
        splatObject.transform.RotateAround(point, normal, UnityEngine.Random.Range(-brush.splatRandomRotation, brush.splatRandomRotation));
    ...
    private void Start()
    {
        ...
        //TargetMaterial.SetTexture("_SplatTex", rt);

        PaintTarget paintTarget = gameObject.GetComponent<PaintTarget>();
        Renderer r = paintTarget.GetComponent<Renderer>();
        RenderTexture rt = (RenderTexture)r.sharedMaterial.GetTexture("_SplatTex");
        ...
        // rt_2 = new RenderTexture(_tex.width, _tex.height, 24, RenderTextureFormat.ARGB32);
    }
    ...
    private void SetupPaint()
    {
        ...
        ClearAllPaint();
       // testScript.GetGroundMat(ref mMat);
    }
    ...
    private void Update()
    {
        ...
        if (timeControl >= 30)
        {
            ...
            var rt_2 = (RenderTexture)render.sharedMaterial.GetTexture("_SplatTex");
            RenderTexture.active = rt_2;
            _tex.ReadPixels(new Rect(0, 0, rt_2.width, rt_2.height), 0, 0);
            _tex.Apply();

            paintCount = 0;
            for (int y = 16; y < rt_2.height; y += rt_2.height /32)
            {
                for (int x = 16; x < rt_2.width; x += rt_2.width / 32)
                {
                    var color = _tex.GetPixel(x, y);
    ...
    void OnGUI()
    {
        ...
        {
           // Graphics.DrawTexture(new Rect(10, 10, 100, 100), rt);
        }
    }
